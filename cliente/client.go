		...
		for key, estructura := range temas.Temas {
			...
			if len(estructura.KeyTema) == 44 && tipoVisibilidad == "publica" { // publica
				...
				if tipo == tipoVisibilidad {
					uiState.ui.Eval(fmt.Sprintf(`$("#getTemas").append('<button type="button" class="btn btn-lg btn-link btn-block" id="%v" onClick="verTema(this)" >%v</button>');`, key, nombre))
				}
				...
				for key := range split {
					if split[key] != "" { // split[key] almacena la key del tema encriptada con la clave publica del usuario
						...
						if temaKey != "" {
							...
							if tipo == tipoVisibilidad {
								uiState.ui.Eval(fmt.Sprintf(`$("#getTemas").append('<button type="button"  class="btn btn-lg btn-link btn-block" id="%v" onClick="verTema(this)" >%v</button>');`, estructura.Id, nombre))
							}
						}
